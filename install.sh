...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        wget -N --no-check-certificate -O /usr/local/x-ui-${arch}.tar.gz https://github.com/imafaz/x-ui/releases/download/${last_version}/x-ui-${arch}.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/imafaz/x-ui/releases/download/${last_version}/x-ui-${arch}.tar.gz"
        echo -e "begin to install x-ui v$1"
        wget -N --no-check-certificate -O /usr/local/x-ui-${arch}.tar.gz ${url}
        if [[ $? -ne 0 ]]; then
    ...
    tar zxvf x-ui-${arch}.tar.gz
    rm x-ui-${arch}.tar.gz -f
    cd x-ui
